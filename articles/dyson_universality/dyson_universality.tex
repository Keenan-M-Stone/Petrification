...
empirically without a theoretical foundation.
...
\href{https://github.com/[repo]}{[GitHub repo]}.
\end{abstract}
...
complex-valued self-consistent field iterations has not
...
\begin{figure}[htbp]
  ...
  \caption{%
    \textbf{Optimal relaxation parameter $\optparam$ vs.\ coupling
    strength $U$.}
    Circles: analytically computed $\optparam$ from
    Eq.~\eqref{eq:thetastar-dyson} at $\omega = i\eta$.
    Dashed line: $\theta = 0.5$.
    The analytical value converges to $1/2$ from above as
    $U \to \infty$, consistent with the wide-band limit
    derivation.
    Inset: convergence iteration count for naive vs.\
    $\theta = 0.5$ across the same $U$ range.
...
DMFT self-consistency loops~\cite{Georges1996}.  Our derivation
provides, for the first time, a rigorous justification for this
